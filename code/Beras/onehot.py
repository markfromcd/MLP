...
class OneHotEncoder(Callable):
    ...
    def fit(self, data):
        ...
        #self.uniq = np.eye(n_values)[data]  # all the unique labels from `data`
        #self.uniq2oh = dict(zip(data, self.uniq))  # a lookup dictionary with labels and corresponding encodings
