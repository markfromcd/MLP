...
class Diffable(Callable):
    ...
    def compose_to_weight(self, J: np.array) -> list:
        ...
        #print(self.weight_gradients())
        for w, wg in zip(self.weights, self.weight_gradients()):
            #print(f'{self.__class__.__name__} compose_to_weight: w', w.shape, 'wg', wg.shape, 'J', J.shape)
            batch_size = J.shape[0]
...
class GradientTape:
    ...
    def gradient(self) -> list:
        ...
        #for operation in self.operations[::-1]:
            #if operation.trainable and hasattr(operation,"weights"):
               #grads += operation.compose_to_weight(input_gradients)
                

            #if input_gradients is None:
                #input_gradients = operation.input_gradients()
            #else:
                #input_gradients = operation.compose_to_input(input_gradients)
        #grads.reverse()
            
        grads = []
        j = self.operations[-1].input_gradients()
        for operation in reversed(self.operations[:-1]):
            if operation.trainable and hasattr(operation, "weights"):
                w = operation.compose_to_weight(j)
                grads = w + grads
            j = operation.compose_to_input(j)
        self.operation = []
          
        #grads = []
        #input_gradients = None
        #i = self.operations[-1].input_gradients()
        #for operation in reversed(self.operations[:-1]):
            #print(f'{self.__class__.__name__} compose_to_weight: w', w.shape, 'wg', wg.shape, 'J', J.shape)
            #if operation.trainable and hasattr(operation,"weights"):
                #print('weights_grad 1', operation.weight_gradients())
                #grads += operation.compose_to_weight(i)
                
            #i = operation.compose_to_input(i)
        #self.operations = []
                #print('weights_grad 2', grads)
                
                #for g in grads
            #print(f'{operation} {input_gradients is None}')
            #print(f'{self.__class__.__name__} compose_to_weight: w', w.shape, 'wg', wg.shape, 'J', J.shape)
            #if input_gradients is None:
                #input_gradients = operation.input_gradients()
            #else:
